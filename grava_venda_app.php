...
if ( isset($_POST['valor_pedido']) ) {
    ...
    try {
        // -----------------------------------------------------------
        $sql_pede_senha = $pdo->query("SELECT * FROM sascard.convenio WHERE codigo = " .  $codigo_convenio);
        while ($row_convenio = $sql_pede_senha->fetch()) {
            ...
            if ($pede_senha == 1) {
                $sql_pede_senha = $pdo->query("SELECT * FROM sascard.c_senhaassociado WHERE cod_associado = '" . $matricula . "' AND id_empregador = ".$empregador." AND senha = '" . $senha . "'");
            ...
            if ($evetivar == true) {
                ...
                if ($qtde_parcelas > 1) {
                    ...
                    for ($as = 1; $as <= $qtde_parcelas; $as++) {
                        ...
                        $sql = "INSERT INTO sascard.conta (associado,convenio,valor,data,hora,mes,empregador,parcela) ";
                    ...
                    $count = 0;
                    $sql = "INSERT INTO sascard.conta (associado,convenio,valor,data,hora,mes,empregador) ";
