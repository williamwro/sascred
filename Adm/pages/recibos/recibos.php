...
$total_liquido = 0;
if ( $_POST["categoria"] != "" ) {
    $query = "SELECT nome_convenio as descri,sum(valor) as total,prolabore,divisao,cod_convenio,id_categoria_recibo 
              FROM sascard.qextrato 
              WHERE mes = '" . $_POST["mes"] ."' 
              AND id_categoria_recibo = " . $_POST["categoria"] . " 
              AND divisao = ".$divisao." 
              AND divulga = 'S' 
              GROUP BY nome_convenio,divisao,prolabore,cod_convenio,id_categoria_recibo
              ORDER BY nome_convenio";
} else {
    $query = "SELECT nome_convenio as descri,sum(valor) as total,prolabore,divisao,cod_convenio,id_categoria_recibo FROM sascard.qextrato WHERE mes = '" . $_POST["mes"] ."' AND divisao = ".$divisao."AND divulga = 'S' GROUP BY nome_convenio,divisao,prolabore,cod_convenio,id_categoria_recibo ORDER BY nome_convenio";
}
$someArray = array();
