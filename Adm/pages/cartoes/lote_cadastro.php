...
$sql_senha = $pdo->query("SELECT MAX(lote) AS lote FROM sascard.lotes_cartao WHERE id_divisao = ".$divisao);
while($row = $sql_senha->fetch()) {
    $lote = (int)$row["lote"]+1;
}

$sql = "INSERT INTO sascard.lotes_cartao(";
...
$sql = "UPDATE sascard.c_cartaoassociado SET data_pedido = :data, lote = :lote WHERE lote isnull";
$stmt = $pdo->prepare($sql);
