...
if (isset($_POST['tipo'])) {
    $tipo = $_POST['tipo'];
    $sql = $pdo->query("SELECT nome FROM sascard.tipoconvenio WHERE codigo = ".$tipo);
    while($row = $sql->fetch()) {
        $nome_tipo = $row['nome'];
    }
}else{
...
$sql_emp = $pdo->query("SELECT * FROM sascard.empregador WHERE id = ".$empregador);
