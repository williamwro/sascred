...
if(isset($_POST['empregador'])) {
    ...
    $sql_empregador = $pdo->query("SELECT nome FROM sascard.empregador WHERE id = ".$empregador_id);
...
if ($cod_tipo != 0 and $empregador_id != "" ) {
    ...
    $query = "Select nome_convenio as descri, sum(valor) as total From sascard.qextrato Where mes = '" . $mes_atual ."' and id_empregador = " . $_POST["empregador"] . " and cod_tipo_convenio = " . $_POST["cod_tipo"] . " and divisao = ".$divisao." Group by nome_convenio, divisao order by nome_convenio";

}else if ($cod_tipo != 0 and $empregador_id == "" ) {
    PDF::setRS("TODOS");
    if ($subtipo == "" || $subtipo == "empregador") {
        $query = "Select nome_empregador as descri, sum(valor) as total From sascard.qextrato Where mes = '" . $mes_atual . "' and cod_tipo_convenio = " . $_POST["cod_tipo"] . " and divisao = ".$divisao." Group by nome_empregador, divisao order by nome_empregador";
    }else{
        $query = "Select nome_convenio as descri, sum(valor) as total From sascard.qextrato Where mes = '" . $mes_atual . "' and cod_tipo_convenio = " . $_POST["cod_tipo"] . " and divisao = ".$divisao." Group by nome_convenio, divisao order by nome_convenio";
    }

}else if ($cod_tipo == 0 and $empregador_id != "" ) {
    PDF::setRS($empregador_nome);
    $query = "Select nome_convenio as descri, sum(valor) as total From sascard.qextrato Where mes = '" . $mes_atual ."' and id_empregador = " . $_POST["empregador"]. " and divisao = ".$divisao." Group by nome_convenio, divisao order by nome_convenio";
}else if ($cod_tipo == 0 and $empregador_id == "" and $subtipo == "CONVENIO") {
    PDF::setRS("CONVENIOS");
    $query = "Select nome_convenio as descri, sum(valor) as total From sascard.qextrato Where mes = '" . $mes_atual . "' and divisao = ".$divisao." Group by nome_convenio, divisao order by nome_convenio";
}else if ($cod_tipo == 0 and $empregador_id == "" and $subtipo == "EMPREGADOR") {
    PDF::setRS("EMPREGADORES");
    $query = "Select nome_empregador as descri, sum(valor) as total From sascard.qextrato Where mes = '" . $mes_atual . "' and divisao = ".$divisao." Group by nome_empregador, divisao order by nome_empregador";
}
PDF::setMS($mes_atual);
$convenio_aux="";
$aux = 0;
