...
if (isset($_POST['valor_pedido']) && isset($_POST['txtSaldoCard'])) {
    ...
    $sql_pede_senha = $pdo->query("SELECT * FROM sascard.convenio WHERE codigo = " . $_POST['cod_convenio']);
    ...
    if ($contador_senha == 1) {
        ...
        $evetivar   = false;
        if ($pede_senha == 1) {
            $sql_pede_senha = $pdo->query("SELECT * FROM sascard.c_senhaassociado WHERE cod_associado = '" . $_POST['matricula'] . "' AND senha = '" . $_POST['pass'] . "'");
            while ($row_senha = $sql_pede_senha->fetch()) {
        ...
        if ($evetivar == true) {
            ...
            if ($nparcelas > 1) {
                ...
                for ($as = 1; $as <= $nparcelas; $as++) {
                    $sql = "INSERT INTO sascard.conta (associado,convenio,valor,data,hora,mes";
                    $sql .= ",empregador,parcela,id_situacao) VALUES (:associado,:convenio,:valor,";
                    $sql .= ":data,:hora,:mes,:empregador,:parcela,:id_situacao) RETURNING lastval()";
                ...
                $sql = "INSERT INTO sascard.conta (associado,convenio,valor,data,hora,mes";
                $sql .= ",empregador,id_situacao) VALUES (:associado,:convenio,:valor,";
                $sql .= ":data,:hora,:mes,:empregador,:id_situacao) RETURNING lastval()";
...
// now try it
$ua = getBrowser();
$sql_acrescenta = $pdo->exec("UPDATE sascard.convenio SET browser='".$ua['name']."' WHERE codigo=".$codigo_convenio);