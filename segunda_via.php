...
$someArray = array();
$std = new stdClass();
$sql = $pdo->query("SELECT * FROM sascard.conta WHERE lancamento = ".$lancamento);
...
$sql = $pdo->query("SELECT * FROM sascard.convenio WHERE codigo = ".$codigo_convenio);
...
$sql = $pdo->query("SELECT codigo, nome, empregador FROM sascard.associado WHERE codigo = '" . $matricula . "' and empregador = ".$empregador);
while ($row_associado = $sql->fetch()) {
    $std->nome          = $row_associado['nome'];
}
$sql = $pdo->query("SELECT cod_verificacao, empregador FROM sascard.c_cartaoassociado WHERE cod_associado = '" . $matricula . "' and empregador = ".$empregador);
while ($row_cartao = $sql->fetch()) {
    $std->codcarteira   = $row_cartao['cod_verificacao'];
}
$as = 1;
if($parcelas > 0) {
    $sql = $pdo->query("SELECT * FROM sascard.conta WHERE associado = '".$matricula."' AND valor = '".$valor."' AND data = '".$data."' AND hora = '".$hora."' AND convenio = ".$codigo_convenio."  AND empregador = ".$cod_empregador. "ORDER BY lancamento");
