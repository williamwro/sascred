...
while($row_assoc = $sql_associado->fetch()) {
    ...
    if ($row_assoc['cod_situacaocartao'] == "1" or $row_assoc['cod_situacaocartao'] == "5" or $row_assoc['cod_situacaocartao'] == "4") {
        ...
        $l_c = 0;
        $sql_debito_associado = $pdo->query("SELECT SUM(valor) AS valor1, mes FROM sascard.conta WHERE mes='" . $m_p . "' AND associado='" . $row_assoc['codigo'] . "' AND empregador = " . $empregador . " GROUP BY mes");
...
$sql = "UPDATE sascard.mes_corrente SET abreviacao = '". $m_p . "'";
$count = $pdo->exec($sql);
if ($contador == 0) {
