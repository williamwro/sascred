...
if (isset($_POST['userconv']) && isset($_POST['passconv'])){
	...
	// VERIFICA SENHA ******************************************************************************************************************************************************
    $sql_conv_senha = $pdo->query("SELECT usuario, senha, cod_convenio FROM sascard.c_senhaconvenio WHERE usuario='".$userconv."' AND senha='".$passconv."'");
    ...
    if( $cod_convenio != 0 ){
        ...
        $ativo = false;
        $sql_conv = $pdo->query("SELECT * FROM sascard.convenio WHERE codigo=".$cod_convenio." AND divulga = 'S'");
    ...
    // now try it
    $ua=getBrowser();
    $sql_log = "UPDATE sascard.convenio SET browser='".$ua['name']."', data_ultimo_acesso='".$dataagora."', hora_ultimo_acesso='".$horaagora."' WHERE codigo=".$cod_convenio;
